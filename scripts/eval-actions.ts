...
const results = await runEval(read('dictionary.json'), read('codelists.json'))
for (const r of results) {
  const mark = r.verdict === 'correct' ? '✓' : r.verdict === 'asked' ? '?' : '✗'
  console.log(`${mark} [${r.lang}] ${r.text.padEnd(45)} expect ${r.expect.padEnd(10)} rules ${String(r.rules).padEnd(10)} overlap ${r.fallback}${r.missing ? '  (expected option not offered!)' : ''}`)
}
